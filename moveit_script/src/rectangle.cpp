...
//Drawing 2 by 1 rectanle (2 units along y-axis, 1 unit along x-axis)
...
int main(int argc, char **argv)
{
  ...
  target_pose.pose.position.y = target_pose.pose.position.y - 1; // Move by -1 along y-axis (to the left)
  ...
  while (ros::ok()){
    ...
    if (abs(current_pose.pose.position.y - target_pose.pose.position.y) < 0.01){
		target_pose.pose.position.x = target_pose.pose.position.x - 1; //Move by -1 along x-axis (backwards)
		move_group.setApproximateJointValueTarget(target_pose); // To calculate the trajectory
		move_group.move(); // Move the robot
		current_pose = move_group.getCurrentPose();

		if (abs(current_pose.pose.position.x - target_pose.pose.position.x) < 0.01){
			target_pose.pose.position.y = target_pose.pose.position.y + 2; //Move by 2 along y-axis (to the right)
			move_group.setApproximateJointValueTarget(target_pose); // To calculate the trajectory
			move_group.move(); // Move the robot
			current_pose = move_group.getCurrentPose();

			if (abs(current_pose.pose.position.y - target_pose.pose.position.y) < 0.01){
				target_pose.pose.position.x = target_pose.pose.position.x + 1; //Move by 1 along x-axis (forward)
				move_group.setApproximateJointValueTarget(target_pose); // To calculate the trajectory
				move_group.move(); // Move the robot
				current_pose = move_group.getCurrentPose();

				if (abs(current_pose.pose.position.x - target_pose.pose.position.x) < 0.01){
					target_pose.pose.position.y = target_pose.pose.position.y - 1; //Return to the initial position to close the rectangle
					move_group.setApproximateJointValueTarget(target_pose); // To calculate the trajectory
					move_group.move(); // Move the robot
					current_pose = move_group.getCurrentPose();
